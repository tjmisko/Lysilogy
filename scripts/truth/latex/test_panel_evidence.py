...
from manual import verify_panel_images
...
def fixture():
    ...
    review = {'schema_version': 1, 'status': 'independently_collected_votes_validated; final_K1_admission_pending', 'packet_sha256': sha256(canonical(packet)), 'prompt_sha256': sha256(prompt), 'scoring_policy_sha256': sha256(canonical(policy)), 'O11_measured': False, 'production_enrichment_run': False}
...
def bind(rows):
    ...
    return bind_panel(candidate, canonical(packet), prompt, [canonical(vote) for vote in votes], canonical(review), canonical(policy), canonical(index), canonical(annotation), {page['image_path']: page['image_sha256'] for page in packet['pages']})
...
class PanelEvidenceTests(unittest.TestCase):
    ...
    def test_should_reject_a_negative_review_when_vote_hashes_are_valid(self):
        for change in ({'status': 'rejected'}, {'findings': ['unresolved provenance']}):
            rows = fixture(); rows[4].update(change)
            with self.subTest(change=change), self.assertRaisesRegex(ValueError, 'accepted disposition'):
                bind(rows)

    def test_should_rehash_the_referenced_panel_images_when_a_packet_substitutes_a_path(self):
        with tempfile.TemporaryDirectory() as directory:
            cache = Path(directory); image = cache/'page.png'; image.write_bytes(b'original image')
            packet = {'pages': [{'image_path': str(image), 'image_sha256': sha256(b'original image')}]}
            self.assertEqual(verify_panel_images(cache, canonical(packet))[str(image)], sha256(b'original image'))
            image.write_bytes(b'substituted image')
            with self.assertRaisesRegex(ValueError, 'actual panel image bytes differ'): verify_panel_images(cache, canonical(packet))
            packet['pages'][0]['image_path'] = str(cache/'missing.png')
            with self.assertRaises(FileNotFoundError): verify_panel_images(cache, canonical(packet))
            packet['pages'][0]['image_path'] = '/outside-cache/page.png'
            with self.assertRaises(ValueError): verify_panel_images(cache, canonical(packet))
            packet['pages'][0]['image_path'] = str(cache/'..'/'.env')
            with self.assertRaises(ValueError): verify_panel_images(cache, canonical(packet))
