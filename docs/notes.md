...
Validation: `cargo test notes::` exercises storage, create-on-open templates, existing-file preservation, API conflicts, traversal/symlink rejection, UTF-8 and size limits, and atomic updates. `npm --prefix web run smoke:notes` exercises the actual CodeMirror editor, old-backend HTML failures and retry, and local-time template creation with synthetic PDF and API fixtures. `npm --prefix web run test:api` checks response validation and timestamp formatting.
