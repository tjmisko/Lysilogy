...
Produce a source-grounded structural map of the entire target paper. Prefer the paper's argumentative or conceptual units over blindly copying every printed heading. Each tile summary must be one or two sentences. Each digest must explain the section's role, central reasoning, evidence, assumptions, and connection to the thesis in language accessible to a smart outsider. Preserve key quotes exactly apart from whitespace and cite the correct PDF page. Do not invent a quote, result, definition, or page number. Distinguish what the authors show from what they merely argue or assume.
